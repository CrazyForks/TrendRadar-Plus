import{a as m}from"./chunk-YRL7WKAS.js";var d=null,r=null,y=null,g=null,u=0,b=null,B=5*60;async function T(){return d||new Promise((e,t)=>{if(window.QRCode){d=window.QRCode,e(d);return}let n=document.createElement("script");n.src="/static/js/lib/qrcode.min.js",n.onload=()=>{d=window.QRCode,e(d)},n.onerror=t,document.head.appendChild(n)})}function c(e){return e>=1e6?(e/1e6).toFixed(1).replace(/\.0$/,"")+"M":e>=1e3?(e/1e3).toFixed(0)+"K":e.toString()}async function L(){if(!m.getUser()){window.Toast?.show("\u8BF7\u5148\u767B\u5F55","error"),window.openLoginModal?.();return}let t=document.getElementById("paymentModal");t||(t=Q(),document.body.appendChild(t)),r=null,b=null,o(),t.classList.add("open"),await k()}function R(){let e=document.getElementById("paymentModal");e&&e.classList.remove("open"),o(),p(),r=null,b=null}function Q(){let e=document.createElement("div");return e.id="paymentModal",e.className="payment-modal",e.innerHTML=`
    ...
    `,e}async function k(){let e=document.getElementById("paymentPlansSection"),t=document.getElementById("paymentCurrentBalance");try{let[n,a]=await Promise.all([fetch("/api/payment/plans"),fetch("/api/payment/balance")]),s=await n.json(),i=await a.json();if(t&&(t.textContent=c(i.total||0)),!s.configured){e.innerHTML=`
        ...
        <div class="payment-plan-card ${t?"recommended":""}" 
            ...
            <div class="payment-plan-tokens">${c(e.tokens)} Tokens</div>
            <div class="payment-plan-desc">\u7EA6 ${n} \u6B21\u603B\u7ED3</div>
        </div>
    `}async function C(e){b=e,document.querySelectorAll(".payment-plan-card").forEach(s=>{s.classList.toggle("selected",s.dataset.planId==e)});let t=document.getElementById("paymentPlansSection"),n=document.getElementById("paymentQRSection"),a=document.getElementById("paymentStatus");t.style.display="none",n.style.display="block",a.textContent="\u6B63\u5728\u521B\u5EFA\u8BA2\u5355...",a.className="payment-status";try{let s=await fetch("/api/payment/create",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({plan_id:e})});if(!s.ok){let l=await s.json();throw new Error(l.detail||"\u521B\u5EFA\u8BA2\u5355\u5931\u8D25")}let i=await s.json();r=i.order_no,document.getElementById("paymentOrderAmount").textContent=i.amount,document.getElementById("paymentOrderTokens").textContent=c(i.tokens),await j(i.code_url),a.textContent="\u7B49\u5F85\u652F\u4ED8...",q(),D()}catch(s){console.error("Create order error:",s),a.textContent=s.message||"\u521B\u5EFA\u8BA2\u5355\u5931\u8D25",a.className="payment-status error"}}async function j(e){let t=document.getElementById("paymentQRCode");t.innerHTML="";try{await T(),new d(t,{text:e,width:200,height:200,colorDark:"#1d1d1f",colorLight:"#ffffff",correctLevel:d.CorrectLevel.M})}catch(n){console.error("QR code error:",n),t.innerHTML='<div class="payment-qr-error">\u4E8C\u7EF4\u7801\u751F\u6210\u5931\u8D25</div>'}}function S(){o(),p(),r=null,document.getElementById("paymentPlansSection").style.display="block",document.getElementById("paymentQRSection").style.display="none",document.getElementById("paymentSuccessSection").style.display="none"}function q(){o();let e=0,t=600;y=setInterval(async()=>{if(!r){o();return}if(e++,e>t){o();let n=document.getElementById("paymentStatus");n.textContent="\u8BA2\u5355\u5DF2\u8D85\u65F6\uFF0C\u8BF7\u91CD\u65B0\u4E0B\u5355",n.className="payment-status error";return}try{let a=await(await fetch(`/api/payment/status?order_no=${r}`)).json();if(a.status==="paid")o(),E(a.tokens_added||a.tokens);else if(a.status==="expired"){o();let s=document.getElementById("paymentStatus");s.textContent="\u8BA2\u5355\u5DF2\u8FC7\u671F\uFF0C\u8BF7\u91CD\u65B0\u4E0B\u5355",s.className="payment-status error"}}catch(n){console.error("Poll status error:",n)}},1e3)}function o(){y&&(clearInterval(y),y=null)}function D(){p(),u=B,w(),g=setInterval(()=>{u--,w(),u<=0&&(p(),N())},1e3)}function p(){g&&(clearInterval(g),g=null)}function w(){let e=document.getElementById("countdownTime");if(!e)return;let t=Math.floor(u/60),n=u%60;e.textContent=`${t}:${n.toString().padStart(2,"0")}`;let a=document.getElementById("paymentCountdown");a&&a.classList.toggle("warning",u<=60)}function N(){o();let e=document.getElementById("paymentStatus");e&&(e.textContent="\u4E8C\u7EF4\u7801\u5DF2\u8FC7\u671F",e.className="payment-status error");let t=document.getElementById("paymentQRCode");t&&(t.innerHTML=`
    ...
    `,F(),e}async function I(){let e=document.getElementById("usageBalanceValue"),t=document.getElementById("usageTotalValue"),n=document.getElementById("rechargeHistoryList"),s=m.getUser()?.id;try{let[i,l,f]=await Promise.all([fetch("/api/payment/balance",{credentials:"include"}),fetch(`/api/payment/usage?user_id=${s}&limit=1`,{credentials:"include"}),fetch("/api/payment/orders?limit=50",{credentials:"include"})]),v=await i.json(),M=await l.json(),P=await f.json();e&&(e.textContent=c(v.total||0)),t&&(t.textContent=c(M.total_consumption||0));let x=(P.orders||[]).filter(h=>h.status==="paid");if(x.length===0){n.innerHTML='<div class="usage-empty">\u6682\u65E0\u5145\u503C\u8BB0\u5F55</div>';return}n.innerHTML=x.map(h=>V(h)).join("")}catch(i){console.error("Load usage data error:",i),n.innerHTML=`
        ...
        <div class="usage-record">
            ...
            <div class="usage-record-tokens recharge">+${c(e.tokens)}</div>
        </div>
    ...
    `,document.head.appendChild(e)}export{R as closePaymentModal,O as closeUsageModal,c as formatTokens,L as openPaymentModal,_ as openUsageModal};
